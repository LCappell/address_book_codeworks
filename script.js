$(document).ready(function(){
    ...
    function addContact(contact){
        ...
        $(".contacts_container .contacts .contact .name .circle").last().css("background-color", `${contact.color}`);
    }
    //colors
    const colors = ["#f4a261", "#e76f51", "#2a9d8f", "#e9c46a", "#e63946", "#a8dadc", "#457b9d", "#1d3557", "#ffb703", "#fb8500", "#ef476f"];
    ...
    $('.add_contact_container form .submit').click(function(e){
        ...
        if(firstName.value === '' || lastName.value === '' || number.value === '' || email.value === ''){
    ...
    //delete contact
    $(document).on('click', '.contacts_container .contacts .contact .delete', function(e){
    ...
    $('header .search_bar input').on('input', function(e){
        ...
        if(data.length > 0){
            ...
            let conactArr = data.filter((contact)=>{
                if(input.length <= contact.firstName.length && input.toLowerCase() === contact.firstName.toLowerCase().substring(0, input.length)){
                    return contact
                }else if(input.length <= contact.lastName.length && input.toLowerCase() === contact.lastName.toLowerCase().substring(0, input.length)){
                    return contact
                }else if(input.length <= contact.number.length && input === contact.number.substring(0, input.length)){
                    return contact;
                }else if(input.length <= contact.email.length && input === contact.email.substring(0, input.length)){
                    return contact;
                }else if(input.length <= contact.firstName.length + contact.lastName.length + 1){
